...
A planned resolver will compute the paper PnL the published position would have realized and post it on Arc after the market resolves, producing a public, monotonic, on-chain track-record. The vault wiring for that resolver lives in `contracts/src/PythiaVault.sol` and is marked **experimental — not in scope for this submission**; it is operator-gated (outsiders cannot deposit) and holds no real user capital.
...
Web env vars (all `NEXT_PUBLIC_*`, all address-shape data, none secret — see `web/.env.local.example`): `UNLOCK_MARKET_ADDRESS`, `USDC_ADDRESS_ARC`, `ARC_CHAIN_ID`, `ARC_EXPLORER_URL`. Client-side reads use the same-origin `/api/rpc` proxy (a read-only allowlist forwarder over the Canteen-issued upstream); wallet transactions are submitted through the connected wallet provider, not through the proxy. `ARC_RPC_URL` itself stays server-side only and is never exposed to the browser bundle.

Open work (post-submission): resolver close-out so resolved markets surface paper PnL on-chain through the experimental `PythiaVault`; live paid-unlock counts on the home traction strip; registered Polymarket V2 bytes32 builder-code attribution proof; CCTP/Gateway revenue bridge wiring.
