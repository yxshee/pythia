...
> This file is the **audit checklist** used by Chrome DevTools MCP and similar
> tools to re-run the post-deploy QA pass. For the concrete release-time proof
> artifacts (commit, build output, test transcripts, live curl evidence),
...
## How to run
Paste this entire file into Claude Code (Chrome DevTools MCP enabled). Or: `claude --print "$(cat VERIFY-CHECKLIST.md)"`.
